...
export interface AzureCredentials extends CloudConnectorCredentials {
...
export class AzureCloudConnector extends BaseCloudConnector {
  ...
  async getSecurityFindings(): Promise<ComplianceFinding[]> {
    try {
      return await this.retryOperation(async () => {
        ...
        const assessments = this.securityClient.assessments.list(
  ...
  private mapAzureSeverity(severity?: string): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private mapAzureStatus(status?: string): 'pass' | 'fail' | 'manual' | 'not-applicable' {
  ...
  private mapAssessmentSeverity(severity?: string): 'low' | 'medium' | 'high' | 'critical' {
    switch (severity?.toLowerCase()) {
      case 'high': return 'high';
      case 'medium': return 'medium';
      case 'low': return 'low';
  ...
  private mapAssessmentStatus(code?: string): 'pass' | 'fail' | 'manual' | 'not-applicable' {
