...
import { 
  ...
  eMASS_POAMExport
} from '../types/atoPackageGeneration';
import { NISTControl, ZTAActivity, POAMItem as CosmosPoamItem } from '../services/cosmosService';
import { ExecutionEnabler, MaturityLevel } from '../types/executionEnablers';
...
export class ATOPackageGenerationService {
   ...
   */
  async generateATOPackage(tenantId: string): Promise<ATOPackage> {
    try {
   ...
   */
  async generateSystemSecurityPlan(tenantId: string): Promise<SystemSecurityPlan> {
    try {
      const tenant = await enhancedCosmosService.getTenant(tenantId);
      ...
      const controls: ControlImplementation[] = nistControls.map(control => ({
      ...
      const systemSecurityPlan: SystemSecurityPlan = {
        systemName,
   ...
   */
  async generatePOAMData(tenantId: string): Promise<POAMItem[]> {
    try {
      const poams = await enhancedCosmosService.getPOAMItems(tenantId);
      const nistControls = await enhancedCosmosService.getNISTControls(tenantId);
      
      // Get non-compliant controls
      const nonCompliantControls = nistControls.filter(
        control => control.overallStatus !== 'compliant'
      );
   ...
   */
  async generateComplianceMetrics(tenantId: string) {
    try {
      const nistControls = await enhancedCosmosService.getNISTControls(tenantId);
      
      // Calculate control family compliance
      const familyMap = new Map<string, { total: number; compliant: number }>();
      ...
      const controlFamilyCompliance: ControlFamilyCompliance[] = Array.from(familyMap.entries()).map(([family, data]) => ({
        family,
      ...
      const continuousMonitoringStatus: ContinuousMonitoringStatus = {
        isActive: true,
        lastAssessmentDate: new Date().toISOString(),
  ...
  async generateZeroTrustAssessment(tenantId: string) {
    try {
      ...
      const pillarReadiness = Array.from(pillarMap.entries()).map(([pillar, activities]) => {
        ...
        let maturityLevel: 'Traditional' | 'Advanced' | 'Optimal' = 'Traditional';
        if (score >= 80) maturityLevel = 'Optimal';
        else if (score >= 60) maturityLevel = 'Advanced';
      ...
      let overallMaturity: 'Traditional' | 'Advanced' | 'Optimal' = 'Traditional';
      if (overallScore >= 80) overallMaturity = 'Optimal';
   ...
   */
  async generateExecutionEnablers(tenantId: string) {
    try {
   ...
   */
  async generateeMASS_POAMData(tenantId: string): Promise<eMASS_POAMExport[]> {
    try {
  ...
  // Helper methods
  private mapComplianceToImplementationStatus(status: string): 'Implemented' | 'Partially Implemented' | 'Planned' | 'Alternative Implementation' | 'Not Applicable' {
    switch (status) {
  ...
  private mapComplianceToRiskLevel(status: string): 'Low' | 'Moderate' | 'High' | 'Very High' {
    switch (status) {
  ...
  private mapPOAMSeverityToRiskLevel(severity: string): 'Low' | 'Moderate' | 'High' | 'Very High' {
  ...
  private mapPOAMStatusToStandard(status: string): 'Open' | 'In Progress' | 'Completed' | 'Risk Accepted' | 'False Positive' {
  ...
  private getRiskPriority(status: string): 'Low' | 'Medium' | 'High' | 'Critical' {
    switch (status) {
      case 'compliant': return 'Low';
  ...
  private estimateRemediationEffort(status: string): string {
    switch (status) {
  ...
  private estimateBusinessImpact(severity: string): 'Low' | 'Medium' | 'High' {
    return this.estimateCostImpact(severity);
  }

  private mapZTAStatus(status: string): 'Not Started' | 'In Progress' | 'Implemented' {
    switch (status) {
      case 'complete':
        return 'Implemented';
      case 'in-progress':
  ...
  private determineSecurityCategorization(controls: any[]): 'Low' | 'Moderate' | 'High' {
  ...
  private calculateOverallRiskLevel(controls: { riskLevel: string }[]): 'Low' | 'Moderate' | 'High' | 'Very High' {
    const riskCounts = {
    ...
    if (riskCounts['High'] > 3) return 'High';
    if (riskCounts['High'] > 0 || riskCounts['Moderate'] > 5) return 'Moderate';
    return 'Low';
  }
  ...
  private mapMaturityToNumber(maturity: number): number {
    return maturity;
  }

  private generateEnablerStatus(name: string, enablers: any[]) {
    ...
    let status: 'Not Addressed' | 'Partially Addressed' | 'Largely Addressed' | 'Fully Addressed' = 'Not Addressed';
    if (score >= 90) status = 'Fully Addressed';
    else if (score >= 70) status = 'Largely Addressed';
    else if (score >= 30) status = 'Partially Addressed';
  ...
  private mapRiskLevelToeMASS(riskLevel: string): 'Very Low' | 'Low' | 'Moderate' | 'High' | 'Very High' {
    switch (riskLevel) {
      case 'Low': return 'Low';
  ...
  private mapStatusToeMASS(status: string): 'Ongoing' | 'Risk Accepted' | 'Completed' | 'Not Applicable' {
    switch (status) {
      case 'Open':
  ...
  private formatDateForeMASS(dateString: string): string {
  ...
  private formatMilestonesForeMASS(milestones: any[]): string {
    return milestones.map(m => 
      `${m.description} (${this.formatDateForeMASS(m.targetDate)})`
    ).join('; ');
  }
