...
class EnhancedRBACService {
     ...
     */
    canApproveAtLevel(userRole, requiredLevel) {
        const rolePermissions = DOD_ROLE_PERMISSIONS[userRole];
     ...
     */
    getRolesForApprovalLevel(requiredLevel) {
    ...
    getApprovalHierarchy() {
        const levels = Array.from(new Set(Object.values(DOD_ROLE_PERMISSIONS)
            .filter(role => role.canApproveExceptions)
            .map(role => role.approvalLevel))).sort((a, b) => a - b);
     ...
     */
    validateClearance(userClearance, resource) {
        if (!userClearance)
            return false;
        ...
        const requiredLevel = Math.min(...requiredClearances.map(clearance => clearanceHierarchy.indexOf(clearance)));
