...
class EnhancedEmassIntegrationService {
     ...
     */
    async syncPoamsFromEmass(tenantId, systemId) {
     ...
     */
    async fetchPoamsFromEmassApi(systemId) {
     ...
     */
    async executeWorkflowRules(tenantId, poam, triggerType) {
        await this.ensureInitialized();
        try {
     ...
     */
    shouldTriggerRule(rule, poam, triggerType) {
     ...
     */
    async executeWorkflowRule(tenantId, rule, poam) {
     ...
     */
    async executeWorkflowAction(tenantId, action, poam) {
        switch (action.type) {
            case 'assign':
     ...
     */
    async incrementRuleExecutionCount(ruleId) {
        try {
     ...
     * Get POA&M by ID
     */
    async getPoamById(tenantId, poamId) {
        await this.ensureInitialized();
        try {
     ...
     * Create new POA&M
     */
    async createPoam(tenantId, poamData) {
        const now = new Date();
        const poam = {
     ...
     * Update existing POA&M
     */
    async updatePoam(tenantId, poamData) {
        const updates = {
            ...poamData,
     ...
     */
    async getWorkflowRules(tenantId, enabledOnly = false) {
        await this.ensureInitialized();
        try {
     ...
     * Log audit activity
     */
    async logAuditActivity(tenantId, activity, metadata) {
        try {
            const auditLog = {
     ...
     */
    async getPoams(tenantId, filters) {
        await this.ensureInitialized();
        try {
            ...
            if (filters?.severity?.length) {
                ...
                parameters.push({ name: '@severities', value: filters.severity });
            }
            ...
            if (filters?.workflowStage?.length) {
                ...
                parameters.push({ name: '@stages', value: filters.workflowStage });
            }
     ...
     */
    async getWorkflowExecutions(tenantId, limit = 50) {
        await this.ensureInitialized();
        try {
            const querySpec = {
    ...
    async getSyncStatistics(tenantId) {
        ...
        try {
            ...
            const now = new Date();
            const overduePoams = allPoams.poams.filter(p => p.scheduledCompletionDate < now && p.status !== 'Completed').length;
            const highSeverityPoams = allPoams.poams.filter(p => p.severity === 'High' || p.severity === 'Very High').length;
            // Get last sync time
