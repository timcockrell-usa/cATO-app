 ...
 */
export class SSCMonitoringService {
    constructor(cosmosClient) {
        this.pipelineContainerName = 'ssc-pipelines';
        this.scanContainerName = 'scan-results';
        this.componentContainerName = 'software-components';
        this.vulnerabilityContainerName = 'vulnerabilities';
        this.configContainerName = 'ssc-configurations';
        this.auditContainerName = 'ssc-audit-trails';
        this.cosmosClient = cosmosClient;
    }
    // Pipeline management
    async getPipelines(organizationId, projectId) {
        try {
    ...
    async executePipeline(pipelineId, triggeredBy) {
        try {
    ...
    // Scan management
    async getScanResults(organizationId, pipelineId, scanType) {
        try {
    ...
    async createScanResult(scanResult) {
        try {
            const container = this.cosmosClient.container(this.scanContainerName);
            const id = this.generateId();
            const now = new Date();
    ...
    async getVulnerabilities(organizationId, severity, status) {
        try {
    ...
    async updateVulnerabilityStatus(vulnerabilityId, status, updatedBy, notes) {
        try {
            const container = this.cosmosClient.container(this.vulnerabilityContainerName);
    ...
    async getComponents(organizationId, componentType) {
        try {
    ...
    async generateComplianceReport(organizationId, framework, timeframe) {
        try {
            const pipelines = await this.getPipelines(organizationId);
            const vulnerabilities = await this.getVulnerabilities(organizationId);
            const components = await this.getComponents(organizationId);
    ...
    async updateSecurityPolicy(organizationId, policyId, updates) {
        try {
    ...
    async triggerPipelineExecution(pipeline, triggeredBy) {
        try {
    ...
    async executeStage(pipeline, stage, triggeredBy) {
        try {
    ...
    async executeScan(pipeline, stage, scanType, triggeredBy) {
        try {
    ...
    async processVulnerabilities(vulnerabilities, organizationId) {
        try {
    ...
    async findExistingVulnerability(vulnId, organizationId) {
        try {
            const container = this.cosmosClient.container(this.vulnerabilityContainerName);
            const { resources } = await container.items
                .query({
    ...
    async createVulnerabilityFromScan(vulnId, organizationId) {
        try {
            const container = this.cosmosClient.container(this.vulnerabilityContainerName);
            const now = new Date();
    ...
    async checkPolicyViolations(scanResult) {
        try {
    ...
    isPolicyViolated(scanResult, policy) {
    ...
    async createPolicyViolationAlert(scanResult, policy) {
        try {
            const alert = {
    ...
    async getRecentScanResults(organizationId, timeframe) {
        try {
            const container = this.cosmosClient.container(this.scanContainerName);
            let query = 'SELECT * FROM c WHERE c.organizationId = @organizationId';
            const parameters = [{ name: '@organizationId', value: organizationId }];
            if (timeframe) {
                ...
                parameters.push({ name: '@startDate', value: timeframe.start.toISOString() }, { name: '@endDate', value: timeframe.end.toISOString() });
            }
            else {
    ...
    async buildDashboardData(organizationId, pipelines, recentScans, vulnerabilities, components, timeframe) {
        const now = new Date();
        ...
        const outdatedComponents = components.filter(c => c.securityStatus === 'Outdated' || c.securityStatus === 'End_of_Life').length;
        const componentsWithVulns = components.filter(c => c.knownVulnerabilities.length > 0).length;
    ...
    calculateVulnerabilityTrends(vulnerabilities) {
    ...
    calculatePatchingMetrics(vulnerabilities) {
    ...
    calculateComponentRiskDistribution(components) {
        return components.reduce((acc, component) => {
            acc[component.riskLevel] = (acc[component.riskLevel] || 0) + 1;
            return acc;
        }, {});
    }
    calculateOverallComplianceScore(pipelines, vulnerabilities, components) {
    ...
    calculateComplianceByFramework(pipelines, vulnerabilities) {
    ...
    calculateOverallRiskScore(vulnerabilities, components) {
    ...
    calculateRiskTrend(vulnerabilities, components) {
    ...
    calculateVulnerabilityAge(vulnerabilities) {
        ...
        if (ages.length === 0)
            return { average: 0, median: 0, maximum: 0 };
    ...
    calculateLicenseCompliance(components) {
        const totalComponents = components.length;
        const compliantComponents = components.filter(c => c.license && !c.license.isRestrictive).length;
        return {
            totalComponents,
            compliantComponents,
    ...
    calculateComponentsByType(components) {
        return components.reduce((acc, component) => {
            acc[component.componentType] = (acc[component.componentType] || 0) + 1;
            return acc;
        }, {});
    }
    calculatePolicyViolations(pipelines, vulnerabilities) {
    ...
    getRecentAuditFindings(organizationId) {
    ...
    calculateRiskByCategory(vulnerabilities, components) {
    ...
    getTopRisks(vulnerabilities, components) {
    ...
    calculateSSCKPIs(pipelines, vulnerabilities, components) {
        return [
            {
    ...
    calculatePipelineSuccessRate(pipelines) {
    ...
    calculateComponentSecurityCoverage(components) {
        if (components.length === 0)
            return 100;
        const monitoredComponents = components.filter(c => c.securityStatus !== 'Unknown').length;
        return Math.round((monitoredComponents / components.length) * 100);
    }
    async buildComplianceReport(organizationId, framework, pipelines, vulnerabilities, components, timeframe) {
    ...
    generateComplianceExecutiveSummary(framework, pipelines, vulnerabilities, components) {
    ...
    generateControlAssessment(framework, pipelines, vulnerabilities, components) {
    ...
    identifyComplianceGaps(framework, pipelines, vulnerabilities, components) {
        return [
            {
    ...
    generateComplianceRecommendations(framework, gaps) {
        return gaps.map(gap => ({
    ...
    mapToSoftwareComponent(resource) {
        return {
            ...resource,
            createdAt: new Date(resource.createdAt),
            updatedAt: new Date(resource.updatedAt),
    ...
    mapToSSCConfiguration(resource) {
        return {
            ...resource,
            createdAt: new Date(resource.createdAt),
            updatedAt: new Date(resource.updatedAt),
            securityPolicies: (resource.securityPolicies || []).map((policy) => ({
                ...policy,
