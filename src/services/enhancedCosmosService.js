...
class EnhancedCosmosDBService {
    ...
    async createCloudEnvironment(environment) {
        try {
    ...
    async getCloudEnvironments(tenantId) {
        try {
            const querySpec = {
    ...
    async updateCloudEnvironmentStatus(environmentId, tenantId, status) {
        try {
    ...
    async createOrUpdateNISTControl(control) {
        try {
    ...
    async getNISTControls(tenantId, environmentIds) {
        try {
            let querySpec;
    ...
    async getNISTControlById(controlId, tenantId) {
        try {
    ...
    async getNISTControlRemediation(controlId, tenantId, cloudProviders) {
        try {
            ...
            if (cloudProviders && cloudProviders.length > 0) {
                ...
                for (const provider of cloudProviders) {
                    if (control.remediation[provider]) {
                        filteredRemediation[provider] =
                            control.remediation[provider];
                    }
                }
    ...
    async createOrUpdateZTAActivity(activity) {
        try {
    ...
    async getZTAActivities(tenantId, environmentIds) {
        try {
            let querySpec;
            if (environmentIds && environmentIds.length > 0) {
                const envPlaceholders = environmentIds.map((_, index) => `@env${index}`).join(',');
    ...
    async getZTAActivityById(activityId, tenantId) {
        try {
    ...
    async getPOAMItems(tenantId, environmentIds) {
        try {
            let querySpec;
            if (environmentIds && environmentIds.length > 0) {
                const envPlaceholders = environmentIds.map((_, index) => `@env${index}`).join(',');
    ...
    async storeComplianceData(data) {
        try {
    ...
    async getLatestComplianceData(tenantId, environmentId) {
        try {
            let querySpec;
            if (environmentId) {
                querySpec = {
    ...
    async getSSCMetrics(tenantId, environmentId) {
        try {
            let querySpec;
            if (environmentId) {
                querySpec = {
    ...
    async createOrUpdateExecutionEnabler(enabler) {
        try {
    ...
    async getExecutionEnablers(tenantId) {
        try {
            const querySpec = {
    ...
    // Utility Methods
    async getOverallComplianceStatus(tenantId) {
        try {
     ...
     */
    convertRevisionFormat(revision) {
        return revision === 'Rev4' ? '4' : '5';
    }
    convertToOrgRevisionFormat(revision) {
        switch (revision) {
            case 'Rev4':
                return 'Rev4';
            case 'Rev5':
    ...
    convertShortToNistRevision(revision) {
        return revision === '4' ? 'Rev4' : 'Rev5';
    }
    /**
     * Set NIST revision for organization
     */
    async setOrganizationNISTRevision(tenantId, revision) {
        try {
     ...
     */
    async getNISTControlsByRevision(tenantId, revision, environmentIds) {
        try {
            let querySpec;
            if (environmentIds && environmentIds.length > 0) {
                const envPlaceholders = environmentIds.map((_, index) => `@env${index}`).join(',');
     ...
     */
    async createOrUpdateRevisionMapping(mapping) {
        try {
     ...
     */
    async performRevisionGapAnalysis(tenantId, targetRevision) {
        try {
            const tenant = await this.getTenant(tenantId);
            if (!tenant) {
                throw new Error(`Tenant ${tenantId} not found`);
            ...
            const currentControls = await this.getNISTControlsByRevision(tenantId, currentTenantRevision);
     ...
     */
    async initiateRevisionUpgrade(tenantId, targetRevision, initiatedBy) {
        try {
            const tenant = await this.getTenant(tenantId);
            if (!tenant) {
                throw new Error(`Tenant ${tenantId} not found`);
     ...
     */
    async getControlRevisionComparison(controlId, tenantId) {
        try {
            // Get current control
            ...
            const mappings = await this.getRevisionMappings(tenantId);
            const mapping = mappings.find(m => m.rev4ControlId === controlId || m.rev5ControlId === controlId);
            if (!mapping) {
    ...
    // ============================================================================
    predictComplianceStatus(currentStatus, changeType) {
        switch (changeType) {
            case 'No Change':
    ...
    calculateMigrationPriority(impact, currentStatus) {
        if (impact === 'high')
            return 'immediate';
        if (impact === 'medium' && currentStatus === 'noncompliant')
            return 'high';
        if (impact === 'medium')
            return 'medium';
        return 'low';
    }
    estimateEffortHours(impact, changeType) {
        const baseHours = {
            'low': 2,
            'medium': 8,
            'high': 24
    ...
    generateMigrationGuidance(mapping) {
        switch (mapping.changeType) {
            case 'Modified':
    ...
    generateRecommendedActions(gapAnalysis) {
        return {
            immediate: [
    ...
    calculateMigrationTime(gapAnalysis) {
    ...
    extractKeyChanges(changeSummary) {
