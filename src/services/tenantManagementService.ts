...
export class TenantManagementService {
  ...
  private async initializeTenantControls(
    tenantId: string, 
    nistRevision: '4' | '5'
  ): Promise<void> {
    try {
      const controlsToCreate = enhancedNISTControlsMultiCloud
        ...
        .map(control => ({
          ...
          tenantId,
          environmentStatus: {},
          overallStatus: 'not-assessed' as const,
      ...
      for (let i = 0; i < controlsToCreate.length; i += batchSize) {
        ...
        await Promise.all(
          batch.map(control => 
            enhancedCosmosService.createOrUpdateNISTControl(control)
          )
        );
      }
  ...
  private async initializeTenantZTAActivities(tenantId: string): Promise<void> {
    try {
      ...
      for (let i = 0; i < activitiesToCreate.length; i += batchSize) {
        ...
        await Promise.all(
          batch.map(activity => 
            enhancedCosmosService.createOrUpdateZTAActivity(activity)
          )
        );
      }
  ...
  private async createTenantKeyVault(tenantId: string): Promise<void> {
  ...
  async removeRole(tenantId: string, userId: string): Promise<Tenant> {
    try {
      const tenant = await enhancedCosmosService.getTenant(tenantId);
      if (!tenant) {
        throw new Error(`Tenant ${tenantId} not found`);
  ...
  getPermissionsForRole(role: DoDAORole): string[] {
    const rolePermissions: Record<DoDAORole, string[]> = {
      'basic-viewer': [
        'view_dashboard',
        'view_controls',
        'view_zta_activities',
  ...
  async addCloudEnvironment(
    tenantId: string, 
    environment: Omit<CloudEnvironment, 'id' | 'tenantId' | 'createdAt' | 'updatedAt'>
  ...
  async getTenantStatistics(tenantId: string): Promise<{
    ...
    totalZTAActivities: number;
    ztaMaturityAverage: number;
    cloudEnvironments: number;
    activeUsers: number;
