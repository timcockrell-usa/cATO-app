...
class SSSCService {
     ...
     */
    async getDashboardMetrics(tenantId, days = 30) {
        try {
        ...
        `,
            ...
            return this.calculateDashboardMetrics(metricsHistory, days);
        }
        catch (error) {
     ...
     */
    calculateDashboardMetrics(metricsHistory, days) {
    ...
    calculateVulnerabilityScore(vulnerabilities) {
        ...
        if (totalVulns === 0)
            return 100;
        const weightedScore = ((vulnerabilities.critical * VULNERABILITY_SCORES.Critical) +
     ...
     */
    calculateVulnerabilityMetrics(latestMetrics, metricsHistory, days) {
     ...
     * Calculate scan metrics
     */
    calculateScanMetrics(metricsHistory) {
     ...
     * Calculate compliance metrics
     */
    calculateComplianceMetrics(metrics) {
     ...
     */
    createTimelineData(metricsHistory, days) {
        const timeline = [];
     ...
     */
    async getVulnerabilityTrends(tenantId, days = 30) {
        try {
            const endDate = new Date();
            const startDate = new Date(endDate.getTime() - (days * 24 * 60 * 60 * 1000));
            const querySpec = {
     ...
     */
    async ingestScanData(tenantId, payload) {
        try {
            // Count vulnerabilities by severity
            const vulnerabilityCounts = payload.scanResults.vulnerabilities.reduce((counts, vuln) => {
            ...
            const policyResults = payload.scanResults.policies.reduce((counts, policy) => {
            ...
            const componentAnalysis = {
                ...
                licenseIssues: payload.scanResults.components.filter(c => c.license.toLowerCase().includes('unknown') ||
                    c.license.toLowerCase().includes('proprietary')).length
            };
     ...
     */
    async getRepositoryScanHistory(tenantId, repositoryName, limit = 50) {
        try {
            const querySpec = {
                query: `
     ...
     */
    async getRepositorySummary(tenantId) {
        try {
            const querySpec = {
                query: `
