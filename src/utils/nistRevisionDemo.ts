...
export class NISTRevisionManagementDemo {
  ...
  private async showCurrentRevisionStatus() {
  ...
  private async performGapAnalysisDemo() {
    ...
    if (gapAnalysis.success) {
      ...
      gapAnalysis.recommendedActions.immediate.forEach(action => 
        console.log(`     • ${action}`)
      );
      console.log('   Short-term:');
      gapAnalysis.recommendedActions.shortTerm.forEach(action => 
        console.log(`     • ${action}`)
      );
    }
  ...
  private async showControlComparisonDemo() {
  ...
  private async initiateUpgradeDemo() {
    console.log('🚀 Step 5: Initiating upgrade demo (Rev4 → Rev5)...');
    
    const upgradeResponse = await nistRevisionService.initiateUpgrade(
      DEMO_TENANT_ID, 
      'Rev5', 
      DEMO_USER_ID
    );
    ...
    if (upgradeResponse.success) {
      ...
      console.log('\n   Next Steps:');
      upgradeResponse.nextSteps.forEach((step, index) => 
  ...
  private async showUpgradeHistoryDemo() {
