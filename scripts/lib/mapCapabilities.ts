...
export interface ZTACapabilityDocument {
  id: string;
  title: string;
  description: string;
  outcome: string;
  ...
  // Additional metadata
  idNumber?: string;
  capabilityPillar?: string;
}

/**
 * Map a CSV row to a ZTA Capability document
 */
export function mapCapabilityRow(row: CSVRow, rowIndex: number): ZTACapabilityDocument {
  try {
    ...
    // Build the document
    const document: ZTACapabilityDocument = {
      id,
      title,
      description,
      outcome,
 ...
 */
export function mapCapabilities(csvData: CSVRow[]): ZTACapabilityDocument[] {
  log(`Mapping ${csvData.length} capability rows to documents`);
  
  const documents: ZTACapabilityDocument[] = [];
 ...
 */
export function validateCapability(capability: ZTACapabilityDocument): boolean {
  const required = ['id', 'title', 'description', 'outcome', 'pillar'];
  
  for (const field of required) {
    if (!capability[field as keyof ZTACapabilityDocument]) {
 ...
 */
export function showCapabilitiesSummary(capabilities: ZTACapabilityDocument[]): void {
