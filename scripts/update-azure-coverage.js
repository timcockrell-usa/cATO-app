 ...
 */
function readAzureCoverageCSV(csvPath) {
  return new Promise((resolve, reject) => {
    console.log(`📂 Reading Azure coverage data from: ${csvPath}`);
    
    if (!fs.existsSync(csvPath)) {
      reject(new Error(`Azure coverage CSV file not found at: ${csvPath}`));
      return;
    }
 ...
 */
function convertToControlUpdates(coverageRecords) {
  const updates = [];
 ...
 */
function updateNISTControlsFile(updates, controlsFilePath) {
  try {
 ...
 */
function generateDatabaseMigration(updates, outputPath) {
 ...
 */
function generateCosmosDBUpdate(updates, outputPath) {
 ...
 */
function generateCoverageReport(updates, outputPath) {
 ...
 */
async function updateAzureControlCoverage() {
  ...
  try {
    ...
    const dirs = ['database', 'scripts', 'docs'];
    dirs.forEach(dir => {
    ...
    const coverageRecords = await readAzureCoverageCSV(csvPath);
